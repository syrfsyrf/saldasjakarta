    ...
    $.ajax({
        ...
        success : function(data){
            if (!$.trim(data)){
                ...
                html2 += '<span><div class="toast toastSuccess mt-3">'+
                ...
                '</div>'+
                '</div></span>';
                html2 += '<span><div class="toast toastFailed mt-3">'+
                '<div class="toast-header">'+
                'Add To Cart'+
                '</div>'+
                '<div class="toast-body" id="errMsg">'+
                'Failed'+
                '</div>'+
                '</div></span>';
                // document.getElementById("name_kat").innerHTML = name_kat;
        ...
        success : function(data){
            if ($.trim(data)){
                ...
                if (data.RESPOND_CODE == '00') {
                    ...
                    var i;
                    for(i=0; i<data.length; i++){
                        if (data[i].OUTcd == '200' || data[i].OUTcd == 200) {
                            $('.toastSuccess').toast('show');
                            getDetailOrder('SUM');
                        } else {
                            $('#errMsg').html(data[i].OUTmsg);
                            $('.toastFailed').toast('show');
                        }
                    }
                }
            }
            /*if (data.OUTcd == 200 || data.OUTcd == '200') {
                // console.log('im TRUE');
                $('.toast').toast('show');
...
function deleteOrder(param){
    $.ajax({
        ...
        success : function(data){
            getDetailOrder('DETAIL');
            sumOrder('DETAIL');
        }
    });
}

function sumOrder(param){
    html = '';
    $.ajax({
        type    : 'ajax',
        url     : base_url+'data/Data_order/sumOrder',
        async   : true,
        dataType    : 'json',
        success : function(data){
                if (!$.trim(data)){   /*
